...
import { useThemeOptions } from "~/builder/hooks/use-theme";
import { CLASS_VALUES } from "~/constants/CLASS_VALUES";
...
export const CLASSES_LIST: ClassListType = {
  ...
  gridRows: {
    classes: map([1, 2, 3, 4, 5, 6, "none"], (v) => `grid-rows-${v}`),
    regExp: "grid-rows-(\\d|none)",
  },
  display: {
  ...
  gap: {
    classes: map(CLASS_VALUES.indent, (v) => `gap-${v}`),
    regExp: "gap-(px|\\d.\\d|\\d|\\[.*\\])",
  },
  gapX: {
    classes: map(CLASS_VALUES.indent, (v) => `gap-x-${v}`),
    regExp: "gap-x-(px|\\d.\\d|\\d|\\[.*\\])",
  },
  gapY: {
    classes: map(CLASS_VALUES.indent, (v) => `gap-y-${v}`),
    regExp: "gap-y-(px|\\d.\\d|\\d|\\[.*\\])",
  },
