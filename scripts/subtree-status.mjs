...
function main() {
  ...
  if (noFetch) {
    ...
    console.log(
      "Warning: --no-fetch — comparing against the last fetched tip (possibly stale or from another remote).",
    );
  } else {
  ...
  if (behind > 0) {
    for (const line of readGit(["log", "--format=  %h %s", `HEAD..${upstreamTip}`])
      .trim()
      .split("\n")
      .slice(0, 5)) {
      console.log(`            ${line}`);
